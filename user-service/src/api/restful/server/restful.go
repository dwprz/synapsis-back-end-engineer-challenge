...
import (
	...
	"user-service/src/api/restful/router"
	"user-service/src/infrastructure/config"
	"net/http"
	"time"
...
func NewRestful(ah *handler.Auth, uh *handler.User) *Restful {
	m := middleware.New()
	...
	router.Auth(app, ah, m)
	router.User(app, uh, m)
